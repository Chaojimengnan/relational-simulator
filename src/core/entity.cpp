    ...
    id_interface(entity_id.str()), pos_(postion)
{
    ...
    {
        trait_weak_map_.add_object(trait_manager.get_trait(trait_id));
    }
}
...
entity::entity(entity&& e) noexcept : id_interface(std::move(e)), trait_weak_map_(std::move(e.trait_weak_map_)), pos_(std::move(e.pos_))
{
}
...
entity& entity::operator=(entity&& e) noexcept
{
    if (this != &e)
    {
        ...
        trait_weak_map_ = std::move(e.trait_weak_map_);
...
{
    trait_weak_map_.call_member_for_every_object(&trait::on_update, this);
}

void entity::on_start()
{
    trait_weak_map_.call_member_for_every_object(&trait::on_start, this);
}

void entity::on_end()
{
    trait_weak_map_.call_member_for_every_object(&trait::on_end, this);
}
