...
{
    auto you = std::make_shared<entity>(entity_id("you"), point(0,0));

    auto my_world = world::instance();
    my_world.add_entity(you);

    EXPECT_EQ(you->get_id(), entity_id("you"));
    
    auto&& my_trait_manager = trait_manager::instance();
    my_trait_manager.add_trait(std::make_shared<temp_trait>(trait_id("temp_trait")));

    you->add_trait(trait_id("temp_trait"));
